(function() {
 ...
 /* Function that sends sql command directly to omnibus sql factory */
 /* Can be used for performing complex sql operations on omnibus */
		var options = {
		   method: 'POST',
		   url: 'http://'+db_config.db_omnibus.host+':'+db_config.db_omnibus.REST_port+'/objectserver/restapi/sql/factory/',
		   headers: {
		       Accept: 'application/json',
		       Authorization: db_config.omni_authorization,
		       'Content-Type': 'application/json',
		       'Content-Length': 64,
		       Host: db_config.db_omnibus.host,
		       Connection: 'keep-alive'
		   },
		   body: {},
		   json: true
		};
		//Example query, change to your own!
		var sqlCmd = db_config.sqlCmd;
		options.body = { sqlcmd: sqlCmd };

		Connection.prototype.sqlCommand = function() {
			request(options, function (error, response, body) {
	 			 if (error) throw new Error(error);
	 			 console.log(body);
	 	 });
	 };
 /* End of sql factory function */
  ...
  Connection.prototype.queryBuilder = function(sql) {
    ...
    if (restOptions.method === 'PATCH') {
      ...
      if (sql.match('.* [Ww][Hh][Ee][Rr][Ee] .*')) {
        ...
        console.log("restOptions.filter");
        console.log(restOptions.filter);
    ...
    if (restOptions.method === 'GET' || restOptions.method === 'DELETE') {
      ...
      if (sql.match('.* [Oo][Rr][Dd][Ee][Rr] [Bb][Yy] .*')) {
        ...
        console.log("SELECT filtermatch:");
