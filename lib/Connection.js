...
/* Redesign in order to accept multiple values in one UPDATE statement!!! */
                fieldsArray.forEach(function(field){
                    ...
                    //console.log('rowsItem = ');console.log(rowsItem);
                    var colsItem = {
                            "type" : collist[fieldName],
                            "name" : fieldName
                    };

                    //console.log('colsItem = ');console.log(colsItem);

                    /* Checks if data type is integer in order to properly format the fieldValue 
                     * (the same should be done for other data types != string */
                    restOptions.payload.rowset.rows[0][fieldName] = fieldValue;
                    if (colsItem.type==='integer'){
                        restOptions.payload.rowset.rows[0][fieldName] = parseInt(fieldValue);
                    }else{
                        restOptions.payload.rowset.coldesc.push(colsItem);
                    }
                    //console.log('restOptions.payload.rowset = ');console.log(restOptions.payload.rowset);
		});
	}
